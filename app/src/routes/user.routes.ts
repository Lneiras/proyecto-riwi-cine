 ...
 * incluida la autenticación completa bajo el prefijo `auth` (HU-007).
 *
 * Endpoints disponibles:
 *  - `POST /users/`                       : Crear un nuevo usuario.
 *  - `GET  /users/`                       : Obtener todos los usuarios.
 *  - `POST /users/auth/register`          : Registro público (HU-006).
 *  - `POST /users/auth/verify-email`      : Activar cuenta con token (HU-006).
 *  - `GET  /users/auth/verify-email`      : Enlace clickeable del correo (HU-006).
 *  - `POST /users/auth`                   : Iniciar sesión (alias: /users/auth/login).
 *  - `POST /users/auth/login`             : Alias explícito de login (HU-007).
 *  - `POST /users/auth/logout`            : Cerrar sesión revocando el refresh token (HU-007).
 *  - `POST /users/auth/forgot-password`   : Solicitar recuperación de contraseña (HU-007).
 *  - `POST /users/auth/reset-password`    : Restablecer contraseña con token (HU-007).
